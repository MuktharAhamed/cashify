...
const UpdateShippingAddress = ({checkout, customer}) => {
  ...
  const [updateShipping, {Shippingloading, ShippingData}] = useMutation(
    ShippingAddressUpdate,
  );
  ...
  const cnfmClick = () => {
    ...
    if (!loading) navigation.navigate(NavCheckoutPage);
  };
      ...
      >
        <View style={{backgroundColor: '#f8f6f9', flexDirection: 'column'}}>
          <Button
            mode="contained"
            onPress={() =>
              navigation.navigate(NavNewShippingAddress, {
                setaddressArr: e => setLineItems(e),
                lineAddress: LineItems,
                setDefaultAddress: e => setDefaultAddress(e),
              })
            }
          >
            Add New
              ...
              <TouchableHighlight>
                <View>
                  <RadioButton
                    value="first"
                    status={
                      DefaultAddress === item.id ? 'checked' : 'unchecked'
                    }
...
export default ShippingAddressUpdatePage = connect(mapStateToProps)(
